...
class etomite {
  ...
  function addPageClass($content,$type="text/html"){
	...
	$alias = '';
	if(empty($doc)){ return $content; }
	if(!empty($doc['alias'])){ $alias = $doc['alias']; }
	if(empty($alias)){ $alias = $doc['id']; }
    // insert the message into the document
    if(strpos($content, "<body>")>0) {
...
$incpath = $_SERVER['DOCUMENT_ROOT'].'/manager/includes/etomiteExtenders';
set_include_path(get_include_path() . PATH_SEPARATOR . $incpath);
// header for weird cookie stuff. Blame IE.
//header('P3P: CP="NOI NID ADMa OUR IND UNI COM NAV"');

// added by John Carlson to add firephp capability
require_once('manager/includes/lib/FirePHPCore/fb.php');
// call is with the fb() functions
// end firephp

function __autoload($class_name) {
	if(!class_exists($class_name)){
    	require $_SERVER['DOCUMENT_ROOT'].'/manager/includes/etomiteExtenders/'.$class_name . '.php';
	}
}
...
include("manager/includes/etomiteExtenders/eto.extender.php");
...
// display firephp to console
if(!empty($etomite->fbDBConn)){
	fb($etomite->fbDBConn,FirePHP::INFO);
}
if(!empty($etomite->fbQueries) && count($etomite->fbQueries)>0){
	array_unshift($etomite->fbQueries, array('SQL Statement','Time','Result'));
	fb(array($etomite->executedQueries." queries took ".sprintf("%2.4f",$etomite->queryTime)." seconds",
	$etomite->fbQueries),FirePHP::TABLE);
}
