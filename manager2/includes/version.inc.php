<?php
// version.inc.php
// Modified: 2008-04-18 [v1.0] by Ralph A. Dahlgren
// Modified: 2008-05-08 [v1.1] by Ralph A. Dahlgren
// Modified: 2012-01-02 [v1.1] by John M. Carlson
$small_version = "2"; // Current codebase
$patch_level = ".0"; // Revision number
$release = "2.0"; // Full current Etomite release number
$code_name = "Raven Alpha"; // Current Etomite codename
$full_appname = "EtoFork - Etomite CMS ".$small_version.$patch_level." ($code_name)";
$full_slogan = "Fast, Free, Infinitely Flexible";
?>