...
setTimeout(autosave,300000); // save every 5 minutes
</script>
...
<?php } ?>
  ...
  <tr>
  	<td align="left">Snippet Section:&nbsp;&nbsp;</td>
    <td align="left"><span style="font-family:'Courier New', Courier, mono">&nbsp;&nbsp;</span><select name="section">
	<?php
		// need to build the section dropdown
		$sqlS = "select * from $dbase.".$table_prefix."site_section where section_type='snippet' order by sort_order,name ASC";
		$rsS = mysql_query($sqlS);
		$limitS = mysql_num_rows($rsS);
		if($limitS>0){
			for($i=0;$i<$limitS;$i++){
				$row = mysql_fetch_assoc($rsS);
				if($row['id']==$content['section']){ $selected=" selected='selected'"; }else{ $selected = ''; }
				?>
