...
class zipcode_class {
   ...
   function get_zip_details($zip) {
      ...
      $sql = "SELECT lat AS lattitude, lon AS longitude, city, county, state_prefix, 
              ...
              FROM ".$this->_eto->dbConfig['dbase'].".zip_code 
              WHERE zip_code='$zip'";
              
      $r = $this->_eto->dbQuery($sql); //mysql_query($sql);
      if (!$r) {
   ...
   function get_zip_point($zip) {
      ...
      $sql = "SELECT lat, lon from ".$this->_eto->dbConfig['dbase'].".zip_code WHERE zip_code='$zip'";
      $r = $this->_eto->dbQuery($sql); //mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
         return false;
   ...
   function get_zips_in_range($zip, $range=_ZIPS_SORT_BY_DISTANCE_ASC, $sort=1, $include_base=true) {
      ...
      $sql = "SELECT zip_code, lat, lon FROM ".$this->_eto->dbConfig['dbase'].".zip_code ";
